...
const CITATION_MANAGER_PANEL_ID = 'jupyterlab-citation-manager:reference-browser';
...
test.describe('Citation styles support', () => {
  ...
  test('find relevant styles', async ({ page }) => {
    ...
    await stylesSelector.locator('.cm-Option:nth-child(1):has-text("Nature")').waitFor();
    ...
    await stylesSelector.locator('.cm-Option:nth-child(1):has-text("APA")').waitFor();
